...
import { GetMyAnswer } from "../../services/survey/survey.service";
import { render } from "@testing-library/react";

export default function Survey() {
    const navigate = useNavigate();
    const { GetSurveyById, PostSurveyAnswer, GetMyAnswer } = useContext(SurveyContext);
    ...
    const getSurveyData = async () => {
        ...
        data.data.questions.map((it, idx) => (it._id = idx));
    ...
    useEffect(() => {
        getSurveyData();
        if (location.state.forCheck === true) {
            setMyAnswer(location.state.myAnswer.answers);
            loadAnswer();
        }
        console.log("location.state.forCheck, ", location.state.forCheck);
    }, []);
    ...
    const loadAnswer = async () => {
        const result = await GetMyAnswer(id, userToken);
        console.log("myanswer", result.answers);
        setMyAnswer(result.answers);
    };

    useEffect(() => {
        console.log("내 앤서", myAnswer);
    }, [myAnswer]);
            ...
            {loaded ? (
                <div className="Survey">
                    <FadeIn className="surveyWrapper" childClassName="childClassName">
                        {location.state.forCheck && <Badge>나의 응답</Badge>}
                            ...
                            {surveyData.questions &&
                                surveyData.questions.map((q, index) => {
                                    ...
                                    return (
                                        <QuestionForm
                                            forCreate={false}
                                            forCheck={location.state.forCheck}
                                            myAnswer={myAnswer}
                                            type={q.type}
                                            q={q}
                                            qIndex={index}
